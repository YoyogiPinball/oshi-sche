...
- 📅 **カレンダー連携**: Googleカレンダーに予定を自動登録（1時間枠で登録）
- 📢 **Discord通知**: 任意のチャンネルへ毎朝◯時に当日・翌日の配信スケジュール一覧を送信
...
#### Discord通知メッセージのフォーマット改善（最新）
- **時刻グループ化**: 同じ時刻の配信をまとめて表示し、スマホでの視認性が向上
- **時間帯絵文字**: 🌌深夜・🌅朝・☀️昼・🌆夕方・🌙夜 で視覚的に時間帯を把握しやすく
- **所属名の明示**: `◯所属：名前` 形式で所属を分かりやすく表示
- **年表示追加**: 日付フォーマットに年を追加（2025/12/14形式）
...
#### フォルダ名の柔軟なパース
- **区切り記号のサポート**: `：` `:` `/` `｜` などを区切り文字として認識
- **VTuber名の保護**: 長音「ー」、ハイフン「-」、中黒「・」は区切り文字から除外し、「星野・ひかり」「桜井マリー」などのVTuber名を正しく認識
...
2. 「新しいプロジェクト」をクリック
3. [src/code.gs](src/code.gs) の内容をコピー＆ペースト
4. 保存（💾アイコンをクリック）
...
- 文字が小さすぎる
- 画像が不鮮明
- 濁点・半濁点が分離している文字（例: 「ペ」が「ペ」のように分かれている）
...
GASのトリガー設定により、毎朝指定した時刻にDiscordへ自動配信されます。
スマートフォンでも見やすいフォーマットで表示されます。

```
📅 **本日の配信スケジュール** 2025/12/14（土）
━━━━━━━━━━━━━━━━━━
🌅 08:00
   ◯個人勢：星野ひかり
   └ 朝雑談配信
...
- [x] **Discord通知機能（実装済み）**: GASのトリガーを設定することで、毎朝6時など任意の時刻に今日・明日のスケジュールを自動配信できます
  - 設定例: `sendDailyScheduleToDiscord` 関数を時間ベースのトリガー（毎日午前6時）で実行
- [ ] WebUIの提供
- [ ] スマートフォンアプリ化（未定）
- [ ] Webアプリ化（未定）
